...
function editorial_services(): array {static $data;if($data!==null)return $data;$data=[];foreach(glob(PROJECT_ROOT.'/content/editorial/services-*.json') as $file){foreach(json_decode(file_get_contents($file),true,512,JSON_THROW_ON_ERROR) as $key=>$value){$key=ltrim($key,'/');if(!dz_route_unpublished($key))$data[$key]=$value;}}return $data;}
